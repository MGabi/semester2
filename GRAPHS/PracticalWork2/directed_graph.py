...
class UndirectedGraph(object):
    ...
    def removeEdge(self, vertexStart, vertexEnd):
        if self.isEdge(vertexStart, vertexEnd):
            ...
            self.__graphOut[vertexEnd].remove(vertexStart)

            self.__graphIn[vertexEnd].remove(vertexStart)
            self.__graphIn[vertexStart].remove(vertexEnd)


            self.__costs.pop(vertexStart, vertexEnd)
            self.__costs.pop(vertexEnd, vertexStart)

            for id, edge in self.__edgeIDs.copy().items():
                if edge == [vertexStart, vertexEnd] or edge == [vertexEnd, vertexStart]:
    ...
    def bfs(self, startingNode):
        visited, queue = set(), [startingNode]
        while queue:
            vertex = queue.pop(0)
            if vertex not in visited:
                visited.add(vertex)
                queue.extend(self.__graphOut[vertex])
        return visited
